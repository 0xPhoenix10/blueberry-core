...
describe('Balancer Spell', () => {
  ...
  describe('Basic', async () => {
    it('wStaking Add Twice Testing', async () => {
      ...
      // console.log('Bob LP Balance: ', await balancerLP.balanceOf(bob.address));
      ...
      let prevAliceDfdBalance = await dfd.balanceOf(alice.address);
      // console.log('Alice DFD Balance: ', prevAliceDfdBalance);

      // console.log(
      //   '========================================================================='
      // );
      // console.log('Case 2. add liquidity the second time');

      prevABal = await dfd.balanceOf(alice.address);
      prevBBal = await dusd.balanceOf(alice.address);
      prevLPBal = await balancerLP.balanceOf(alice.address);
      prevLPBalBank = await balancerLP.balanceOf(blueberryBank.address);
      prevLPBalStaking = await balancerLP.balanceOf(staking.address);

      prevARes = await balancerPool.getBalance(dfd.address);
      prevBRes = await balancerPool.getBalance(dusd.address);

      dfdAmt = BigNumber.from(10).pow(18).mul(58000);
      dusdAmt = BigNumber.from(10).pow(18).mul(42000);
      lpAmt = BigNumber.from(0);

      totalDfdAmt = dfdAmt.add(borrowDfdAmt);
      totalDusdAmt = dusdAmt.add(borrowDusdAmt);

      ratio1 = new Decimal(prevARes.toString())
        .add(new Decimal(totalDfdAmt.toString()))
        .div(new Decimal(prevARes.toString()))
        .pow(dfdWeight);

      ratio2 = new Decimal(prevBRes.toString())
        .add(new Decimal(totalDusdAmt.toString()))
        .div(new Decimal(prevBRes.toString()))
        .pow(dusdWeight);

      ratio = ratio1.mul(ratio2).sub(1);
      lpTotalSupply = await balancerLP.totalSupply();
      lpDesired = lpAmt.add(
        BigNumber.from(
          new Decimal(lpTotalSupply.toString())
            .mul(ratio)
      ...
      curABal = await dfd.balanceOf(alice.address);
      curBBal = await dusd.balanceOf(alice.address);
      curLPBal = await balancerLP.balanceOf(alice.address);
      curLPBalBank = await balancerLP.balanceOf(blueberryBank.address);
      curLPBalStaking = await balancerLP.balanceOf(staking.address);

      curARes = await balancerPool.getBalance(dfd.address);
      curBRes = await balancerPool.getBalance(dusd.address);
      ...
      bankInfo = await blueberryBank.getBankInfo(dfd.address);
      ...
      let curAliceDfdBalance = await dfd.balanceOf(alice.address);
      // console.log('Alice Dfd Balance: ', curAliceDfdBalance);
      let receivedDfd = curAliceDfdBalance.sub(prevAliceDfdBalance).add(dfdAmt);
      // console.log('Received Dfd: ', receivedDfd);
      ...
      expect(receivedDfd).to.be.equal(receivedDfdFromStaking);
    }).timeout(1000000);
  });
});
